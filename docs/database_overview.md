...
| 账户与持仓层 | `public.portfolio_asset`、`public.portfolio_positions`、`public.portfolio_changes` | 保存账户资金和持仓当前状态，并通过 append-only 审计日志保留历史变化。 |
...
外部账户同步脚本
  -> public.portfolio_asset / portfolio_positions
...
`portfolio_asset` 和 `portfolio_positions` 是按主键覆盖的当前状态表。
`portfolio_changes` 是 append-only 审计日志，只有它具有账户资金和持仓变化的
时间序列语义。同步脚本通过 `content_hash` 检测状态是否变化。
...
| `public.portfolio_asset` | 每个账户最新资金快照 | 当前现金、冻结资金、持仓市值和总资产查询。 |
| `public.portfolio_positions` | 每个账户、每只股票最新持仓快照 | 当前持仓、可卖数量、成本和市值查询。 |
| `public.portfolio_changes` | 账户和持仓变更审计日志 | 资金曲线、持仓历史、增删改追踪。 |
...
```sql
SELECT detected_at, change_type, stock_code, snapshot
